...
const BookCard = ({book}: { book: IBook }) => {
    const authorNames = book.authors.map((author) => `${author.firstName} ${author.lastName}`).join(", ");

    return (
        <div className="bg-white border rounded-lg shadow-md p-4 cursor-pointer">
            <Link href={`/books/${book.id}/${book.title.replace(/\s+/g, '-')}`} passHref>
                    ...
                    <div className="flex items-center mt-2">
                        {[...Array(5)].map((_, index) => (
                            <svg
                                ...
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                className={`w-5 h-5 ${index < Math.floor(book.rating) ? "text-yellow-400" : "text-gray-400"}`}
                                strokeWidth="2"
                            >
                                <path
                                    fill="currentColor"
